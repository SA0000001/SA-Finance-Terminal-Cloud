...
def turev_cek():
    ...
    # ── OKX Büyük İşlem (Whale) Verisi ───────────────────────────────────────
    # OKX top trader long/short ratio — mevcut LS verisinin kurumsal katmanı
    # Büyük hesap ağırlıklı pozisyonu gösterir (normal L/S'den farklı)
    whale_response = None
    try:
        whale_response = safe_fetch_json(
            "OKX Whale",
            "https://www.okx.com/api/v5/rubik/stat/contracts/long-short-account-ratio-contract-top-trader?instId=BTC-USDT-SWAP&period=1H",
            timeout=6,
            headers=HEADERS,
        )
        wdata = whale_response.payload.get("data", [])
        if wdata:
            first = wdata[0]
            if isinstance(first, dict):
                long_r = float(first.get("longRatio", 0.5))
                short_r = float(first.get("shortRatio", 0.5))
            else:
                long_r = float(first[1]) if isinstance(first, list) and len(first) > 1 else 0.5
                short_r = 1 - long_r
            whale_ratio = long_r / short_r if short_r > 0 else 1.0
            whale_long_pct = long_r * 100
            whale_short_pct = short_r * 100
            net_bias = (long_r - short_r)  # -1 ile +1 arası
            net_bias_usd_approx = net_bias * 5_000_000  # ~$5M scale (OKX top trader hacmi tahmini)

            data["HL_NET_BIAS_RAW"] = net_bias_usd_approx
            data["HL_NET_BIAS"] = f"+%{whale_long_pct:.1f} Long" if whale_long_pct > 50 else f"+%{whale_short_pct:.1f} Short"
            data["HL_LONG_VOL"] = f"%{whale_long_pct:.1f}"
            data["HL_SHORT_VOL"] = f"%{whale_short_pct:.1f}"
            data["HL_WHALE_ALERT"] = (
                "🐋 NET LONG" if whale_ratio > 1.15
                else "🐋 NET SHORT" if whale_ratio < 0.87
                else "Nötr"
            )
            data["HL_BIG_TRADES"] = f"{whale_ratio:.3f} L/S"
            health.success("OKX Whale", whale_response.latency_ms, stale_after_seconds=300)
        else:
            raise ValueError("OKX Whale data empty")
    except FetchError as exc:
        _record_fetch_error(health, "OKX Whale", exc, stale_after_seconds=300)
        data.setdefault("HL_NET_BIAS_RAW", None)
        data.setdefault("HL_NET_BIAS", PLACEHOLDER)
        data.setdefault("HL_LONG_VOL", PLACEHOLDER)
        data.setdefault("HL_SHORT_VOL", PLACEHOLDER)
        data.setdefault("HL_WHALE_ALERT", PLACEHOLDER)
        data.setdefault("HL_BIG_TRADES", PLACEHOLDER)
    except DATA_PARSE_EXCEPTIONS as exc:
        _record_parse_error(
            health,
            "OKX Whale",
            exc,
            latency_ms=whale_response.latency_ms if whale_response else None,
            stale_after_seconds=300,
        )
        data.setdefault("HL_NET_BIAS_RAW", None)
        data.setdefault("HL_NET_BIAS", PLACEHOLDER)
