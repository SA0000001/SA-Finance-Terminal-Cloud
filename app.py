...
# ── Floating sidebar toggle — components.html ile gerçek DOM erişimi ─────────
components.html("""
<style>
#sa-sidebar-toggle {
  position: fixed;
...
#sa-sidebar-toggle:hover svg { opacity: 1; }
</style>

<div id="sa-sidebar-toggle" title="Menüyü aç / kapat">
  <svg id="sa-icon" width="10" height="16" viewBox="0 0 10 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path id="sa-arrow" d="M7 2L2 8L7 14" stroke="#52c8ff" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>
</div>
...
(function() {
  ...
  function getSidebarBtn() {
    // Sidebar açıkken içindeki collapse butonu
    var sidebar = P.querySelector('[data-testid="stSidebar"]');
    if (sidebar) {
      var btn = sidebar.querySelector('button');
      if (btn) return btn;
    }
    // Sidebar kapalıyken collapsed control
    return P.querySelector('[data-testid="collapsedControl"]');
  }

  function isSidebarOpen() {
    var s = P.querySelector('[data-testid="stSidebar"]');
    return s ? s.getBoundingClientRect().width > 80 : false;
  }

  function updateArrow(open) {
    var arrow = document.getElementById('sa-arrow');
    if (!arrow) return;
    // Açıksa sol ok (kapat), kapalıysa sağ ok (aç)
    arrow.setAttribute('d', open ? 'M7 2L2 8L7 14' : 'M3 2L8 8L3 14');
  }

  document.getElementById('sa-sidebar-toggle').addEventListener('click', function() {
    var btn = getSidebarBtn();
    if (btn) {
      btn.click();
      setTimeout(function() { updateArrow(isSidebarOpen()); }, 200);
    }
  });

  // Başlangıç ok yönü
  setTimeout(function() { updateArrow(isSidebarOpen()); }, 800);
})();
</script>
""", height=0)
